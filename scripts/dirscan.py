            ...
            try:
                ...
                if s in [200, 301, 302, 303]:
                    ...
                    while True:
                        try:
                            # 执行sql语句
                            sql = "SELECT * FROM result where id = %d" % uid
                    ...
                    url = urllib.request.urlopen(scan_url).read()
                    html = BeautifulSoup(url)
                    # title = html.find('title')
                    title = html.title.string
                    print(title)
                    """
                    page = urllib.request.urlopen(scan_url)
                    ...
                    try:
                        cur.execute(
                            "insert into result values('%d','%s','%s','%d')" % (uid, title, scan_url, s))
...
def main():
    ...
    parser.add_argument("-w", dest="wordlist", type=str, default="./dic/wordlist.txt",
